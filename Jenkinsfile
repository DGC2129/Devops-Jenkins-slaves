...
pipeline {
    agent none
    environment {
        DUMMYENV = "WELCOME TO DEVOPS B26 BATCH"
        EMAIL = "sreeharshav@gmail.com"
    ...
    stages {
        stage('Deploy To Development') {
            agent { label 'DEV' }
            environment {
            DEVDEFAULTAMI = "ami-0b7005e58315b2d84"
            ...
            stages {
                stage('Perform Packer Build') {
                    ...
                    steps { //Assume Role in SreeMasterAccount AWS Account
                        withAWS(role:'DevOpsB26JenkinsAssumeRole', roleAccount:'053490018989', duration: 900, roleSessionName: 'jenkins-session') {
                            sh 'pwd'
        ...
        //            }
        //            steps { //Assume Role in SreeMasterAccount AWS Account
        //                withAWS(role:'DevOpsB26JenkinsAssumeRole', roleAccount:'009412611595', duration: 900, roleSessionName: 'jenkins-session') {
        //                    sh 'pwd'
        //                    sh 'rm -f prod-backend.tf'
        //                    sh 'ls -al'
